...
execute 'Unpack zookeeper from download' do
  ...
  not_if { ::File.exist?("#{download_path}/zookeeper-#{node['zookeeperd']['jar']['download']['version']}") }
end
...
execute 'Install zookeeper files' do
  ...
  not_if { ::File.exist?("#{node['zookeeperd']['jar']['install_dir']}/zookeeper-#{node['zookeeperd']['jar']['download']['version']}") }
end
